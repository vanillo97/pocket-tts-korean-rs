...
/// Downmix multi-channel audio [C, T] to mono [1, T] by averaging channels.
/// Matches `d.reshape(-1, ch).mean(axis=1)` in deploy_package enroll_ov.py.
pub fn to_mono(audio: &Tensor) -> anyhow::Result<Tensor> {
    let shape = audio.dims();
    if shape.len() != 2 {
        anyhow::bail!(
            "Expected audio tensor with shape [channels, samples], got {:?}",
    ...
    let vec2 = audio.to_vec2::<f32>()?;
    let channels = vec2.len();
    let samples = vec2[0].len();
    ...
    Ok(Tensor::from_vec(mono, (1, samples), audio.device())?)
}
